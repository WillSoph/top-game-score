...
import { doc, serverTimestamp, setDoc, getDoc, updateDoc, increment } from 'firebase/firestore';
...
export default function PlayPage() {
  ...
  async function submit(i: number) {
    ...
    try {
      ...
      if (next < questions.length) {
        ...
        // acabou o quiz → mostra ranking local
        setShowRanking(true);
      }
    } catch (e: any) {
  ...
  return (
    ...
    <div className="max-w-3xl mx-auto p-6 space-y-6">
      <button onClick={() => history.back()} className="text-sm text-slate-400">
        ← Back
      </button>
      ...
      {!joined ? (
        ...
        >
          <h2 className="text-xl font-semibold mb-4 text-center">🏆 Final Ranking</h2>
        ...
        <div className="text-slate-400">Waiting for the host to open the quiz…</div>
      ) : (
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <LiveQuestion
                q={currentQuestion}
                onChoose={submit}
                roundStartedAt={{ toMillis: () => roundStart ?? Date.now() }}
